...
userRouter.get("/request/recieved", userAuth, async (req, res, next) => {
    try {
        ...
        const data = await ConnectionRequest.find({
            ...
            status: "interested"
        }).populate("fromUserId", "firstName lastName age gender skills bio photo")
        res.status(200).json({
            success: true,
...
userRouter.get("/view/connection", userAuth, async (req, res, next) => {
    try {
        ...
        }).populate("fromUserId", "firstName lastName age gender skills bio photo").populate("toUserId", "firstName lastName age gender skills bio photo")
